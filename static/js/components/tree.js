window.TreeComponent = {
  render: function(containerId, causalData, onWhatIfChange, onNodeClick) {
  ...
  _renderSVGTree: function() {
    ...
    edges.forEach(function(edge) {
      ...
      svgHtml += '<line x1="' + fromPos.x + '" y1="' + (fromPos.y + 35) + '" x2="' + toPos.x + '" y2="' + (toPos.y - 10) + '" ' +
    ...
    for (var i = 0; i < nodeKeys.length; i++) {
      ...
      var isSelected = this._selectedNodeKey === key;
      var borderColor = isSelected ? '#38BDF8' : (isCritical ? '#EF4444' : (isWarning ? '#F59E0B' : '#10B981'));
      ...
      svgHtml += '<g class="tree-node-group" data-node-key="' + key + '" style="cursor: pointer;" transform="translate(' + (pos.x - 70) + ', ' + (pos.y - 10) + ')">' +
        '<rect width="140" height="65" rx="8" ry="8" fill="rgba(17, 24, 39, 0.95)" stroke="' + borderColor + '" stroke-width="' + (isSelected ? 3 : (isCritical ? 2.5 : 1.5)) + '" />' +
    ...
    const self = this;
    const nodeGroups = svgArea.querySelectorAll('.tree-node-group');
    nodeGroups.forEach(function(g) {
      g.addEventListener('click', function() {
        const nodeKey = g.getAttribute('data-node-key');
        self._selectedNodeKey = nodeKey;
        self._renderSVGTree();
        if (self.onNodeClick) {
          self.onNodeClick(nodeKey);
        }
      });
    });
  },
